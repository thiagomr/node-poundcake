...
program
    ...
    .option('--swagger', 'add swagger doc')
...
function createApp(options) {
    try {
        ...
        if (options.mongo || options.rabbitmq) {
            fs.mkdirSync(`${options.name}/src/services`);
        ...
        if (options.swagger) {
            packageFile.dependencies['swagger-ui-express'] = '^4.1.4';

            fs.mkdirSync(`${options.name}/src/config`);
            fs.copyFileSync(`${templatesDir}/common/swagger.json`, `${options.name}/src/config/swagger.json`);
        }
